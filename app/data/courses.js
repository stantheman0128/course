        ...
        const CHANGELOG = [
            {
                ...
                type: 'patch',
                changes: [
                    '**學分計算修正：分類封頂＋超修溢流**。原本只有通識會把超過18學分的部分溢流到自由選修，其他分類（例如只需6學分、卻已修7學分的系選修）再模擬選課時會把學分直接灌進分類與畢業總學分。現在每個分類都封頂，超修學分一律溢流到*自由選修*；自由選修本身也封頂27學分，超過所有上限的學分__不計入128畢業總學分__',
                    '**系選修超修顯示**：系選修改顯示 *6/6*，超修的部分列進自由選修的「系選修超修學分」項目（與通識超修同一種呈現）；基線總學分維持 94 不變，只是分佈更正',
                    '**修正課名衝突**：114-2 可模擬的「體育（籃球初級）」與 113-2 已修的同名課程衝突（模擬選課用課名當 key），改名為*體育（籃球初級）114-2*',
                    '**頁首統計單一來源**：index.html 原本寫死 94/128/34/73.4% 與 app.js 內數值重複、可能不同步，改由 `initStats()` 於載入時從同一份計算基線填入',
                    'service worker 快取版本 bump 到 *course-v2.0.6*、資源連結 cache-busting query 同步更新'
                ]
            },
            {
                version: 'v2.0.5',
